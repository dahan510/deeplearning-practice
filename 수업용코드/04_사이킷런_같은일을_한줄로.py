...
# 선형회귀(와 그 사촌 Ridge)는 공식이 있고, 로지스틱 회귀 부터는 사이킷런도 속으로 걷습니다.
# (max_iter 가 그 흔적)
print("    R²:", round(model.score(X1, y), 4), "(01 의 0.7989)")

# 가장 흔한 에러 미리보기 X를 한 줄 (1차원)로 넣으면:
try:
...
# 02의 "섞고 70:30으로 자르기"가 이 한 줄. 돌려주는 순서 (X학습, X시험, y학습, y시험)을 외우세요. 순서를 바꾸면 조용히 망함.
# random_state=42 = 02의 RandomState(42)와 같은 역할(재현성). 단, 섞는 방식이 달라 02와 '같은 140대'는 아닙니다
...
# 02와 분할이 달라 숫자는 조금 다르지만 그림은 같습니다: 공기온도가 압도적(+1.99), 나머지 거의 0
...
# 시험용이 60대뿐이라 흔들리는 정도.
# 진짜 과접합인지 확인하려면 교차검증에서 다시 봅니다 밑에서.
...
# 왜? fit 할땐 학습용으로 스케일러를 맞추고, predict 할 땐 변환만 하는 걸 도구가 알아서 해 줌.
# 사람이 실수로 scaler.fit(X_test)를 못 하게 구조로 막는 것.
...
# 누수 = 시험용 데이터의 정보가 학습 쪽으로 새어들어가는 것. 점수가 부풀려지고, 현장에서 무너짐.
# 제일 흔한 사고 : 00 에서 배운 '중복 행 제거'를 깜박한 경우.
# 같은 측정이 2번  들어있으면 무작위로 나눌때 쌍둥이가 학습용과 시험용에 갈라져 들어갑니다
# 그럼 모델은 시험 문제를 이미 학습용에서 본 셈이 됩니다

from sklearn.neighbors import KNeighborsRegressor

# KNeighborsRegressor(1) = "제일 비슷한 설비 한 대를 찾아 그 설비의 정답을 그대로 말하는" 단순한 모델.
...
# "개발 할땐 0.9 나왔는데 현장에 넣으니 0.4"라는 대형 사고,
# 원인의 절반이 이겁니다. 데이터를 늘리려고 중복을 안 지웠거나,
# 같은 설비를 여러 번 잰 걸 다른 데이터로 취급했거나.

# 현장에 넣으면 어떤 점수가 나오냐? 0.19 입니다. 0.82는 존재하지 않는 실력이었다
# '개발할 땐 잘 나왔는데 현장에선 안 되네요' 사고의 절반이 이 종류입니다.
# 막는 법 2가지:
# 1) 나누기 전에 중복을 지운다. 같은 설비/같은 시각이 2번 있으면 의심할 것.
# 2) 표준화같은 전처리는 반드시 학습용이라만 fit한다. -> 그걸 자동으로 지켜주는 게 위의 Pipeline
# 현장 배포 형태가 바로 이거. 센서 원값 4개를 넣으면 답이 나오는 '한 덩어리'. 실무코드는 거의 이 형태
...
# predict_proba는(60,2) 표:열 0 =정상일 확률, 열 1 = 고장일 확률. [:,1] = 모든 행의 1번 열
판정 = clf.predict(
    Xc_test
...
# 03 손코드의 1등 확률은 0.528이었는데 여긴선 0.366 왜 낮나?
# 사이킷런 LogisticRegression은 기본으로 "규제"가 켜져 있어 가중치를 살짝 눌러 확률이 덜 극단적으로 나옵니다.
# (분할도 다릅니다.) 값은 달라도 "고장확률이 전반적으로 낮게 나온다"는 그림은 같습니다.
...
# ravel() = 2x2 표를 한 줄 4개로 폄. 왼쪽 위부터 tn, fp, fn, tp
...
# 읽는 법: 행 = 편(정상/고장). precision = 정밀도, recall = 재현율
# f1-score = 둘의 평균 같은 것, support = 그 편의 실제 대수.
# '고장'행 recall 0.00 = 고장 2대 중 0대 잡음. 이 한 칸이 제조에서 제일 중요한 숫자.
# '정상'행이 아무리 좋아도 소용없습니다. 아래 accuracy 0.97은 03에서 배운 그 거짓말.
# zero_division = 0 =0으로 나눌 때 경고 대신 0을 쓰라는 뜻(고장을 하나도 안 잡으면 정밀도 분모가 0)
...
# 0.5, 0.3 에선 전멸, 0.1까지 내려야 둘 다 잡습니다(재현율 1.0, 정밀도 0.17 = 고장이라 한 12대 중 진짜는 2대)
# 처방 1 : 임계값 내리기 (위 표). 학습은 그대로 두고 자르는 선만 옮김.
# 처방 2 : 학습할 때부터 "고장 1대를 정상 약 22대만큼 무겁게 세라"고 시키기 = class_weight="balanced"
# (학습용에 정상 134: 고장 6 -> 134/6 ~= 22배. 사이킷런이 이 비율을 자동으로 계산해 벌점에 곱함)
...
# 규제 없음 : train 0.89/ test 0.43 -> 차이 0.46 전형적 과적합 학습 점수는 올랐는데 시험 점수
# Ridge 10: train 0.87/test 0.65 -> 학습 점수를 조금 양보하고 시험 점수를 되찾음. 이게 규제가 하는 일.
# 그래도 원래 4개 특징 (0.74)보다 못합니다. 이 데이터는 직선이 정답이라 부풀릴 이유가 없었던 것.
# 정리 : train>>test가 보이면 -> 규제(alpha 올려) / 데이터 더 모으기/ 특징 줄익.
...
# 조각마다 0.71~0.85 로 흔들립니다. train/test 차이가 0.073은 이 흔들림 안에 있음.
# 과적합 아님, 데이터가 적어서 튀는 것.
# 이렇게 교차검증은 "그 차이가 진자 과적합인지, 우연인지"를 가려 줍니다.
