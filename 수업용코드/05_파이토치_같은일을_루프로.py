...
# =====================================================================
# 파이토치 = 딥러닝 도구 장치
# - 사이킷런이 'fit 한 줄에 다 해 주는 완성푸머'이라면, 파이토치는 우리가 01에서 짠 루프를 그대로 쓰
#   '밟아보기

# 1. 데이터가 아주 커서 조금씩 나눠 학습해야 할 때
# 2. 학습 과정(손실 보폭 걸음 수)을 내 손으로 조절해야 할때
# 3. 다음 과정에서 배울
# 표데이터 +선형모델이면
import os
import numpy as np
...
# =====================================================================
# 텐서(tensor) = 파이토치의 배열. numpy 배열과 거의 같고, '미분 추적 기능'이 붙어 있는 게 차이
...
# 손잡이 5개면 5번, 100만개면 100만번.
# 파이토치는 계산과정을 '기록'해 뒀다가 backward() 한 번에 모든 손잡이의 기울기를 내놓습니다.
# autograd(자동 미분) = 계산 과정을 기록해 두고 backward() 한 번으로 모든 손잡이의 편미분을 구해 주는 기능
# 파이토치를 쓰는 첫 번째 이유. 손잡이가 몇 백만 개인 신경망에선 밟아보기 불가능해서 이게 필수
...
# 01 방식(밟아보기)으로 대조 (밟아보기는 아주 작은 차이로 재니, 정밀한 numpy float64로)
...
# 주의.grab는 backward 할 때마다 '더해집니다'. 매 걸음 전에 비워야 합니다.
...
# with torch.no_grad(): 이 블록 안에서 기록을 끕니다. 예측만 할땐 관례
with torch.no_grad():
    print(
        "\n[4] 다변수 회귀 — train R²",
    ...
    # weight 는 (1,4) 표 모양. sqeeze(0) = 크기 1인 겉껍질을 벗겨 (4,)로. .tolist() = 파이썬 리스트로
print("    04 사이킷런: train 0.8155 / test 0.7425, 공기온도 1.986 ← 같은 자리")
# 사이킷런 공식으로, 파이토치는 500걸음 걸어서. 같은 바닥에 도착. 과적합 판단도 04 같습니다.

# =====================================================================
# 5. 분류 — 03 을 파이토치로: 손실 이름 하나만 바뀐다
# =====================================================================
# 03에서 배운 두 가지 변화(sigmoid, 로그손실)를 파이토치는 손시 ㄹ함수 하나에 담아둡니다:
# torch.nn.BCEWithLogitLoss() = "직선값(logit)을 받아 안에서 sigmoid를 씌우고 로그손실(BCE) 계산"
# (BCE = Binary Cross-Entropy) = 03의 로그손실. 같은 것의 다른 이름\
# 층(nn.linear) 옵티마이저 네 줄 루프는 회귀와 완전히 같습니다. "분류 = 회귀 + sigmoid + 손실 교체"가 여기서도 그대로.
...
# 파이토치엔 규제가 없어 확률이 더 벌어집니다.
# 하지만 1등(0.539)은 진짜 고장이 아니라 헉경보고, 임계값 0.5에선 여전히 고장을 못 잢브니다. 0.2로 내려야 1대 잡는다
# 도구가 달라도 결론은 같습니다. 불균형 데이터는 class_weight로 손 봐야함 (04)
# 파이토치에서 class_weight는 BCEWithLogitLoss(pos_weight=torch.tensor([22, ]))로

# =====================================================================
# 6. 미니배치 — 데이터가 클 때 조금씩 나눠 걷기 (파이토치를 쓰는 두 번째 이유)
# =====================================================================
# 지금까진 매 걸음 140대 전부로 기울기를 쟀습니다. 데이터가 100만 건이면 한걸음이 너무 무겁고 메모리에 안 들어감
# 그래서 16대씩 뽑아 한 걸음, 다음 16대로 한 걸음... 이렇게 갑니다. DataLoader 가 섞고 나눠 줍니다.
# 배치 = 한 걸음에 보는 데이터 묶음. 미니배치 = 그 묶음이 전체의 일부일 때.
# 에폭 = 전체를 한 바퀴 다 보는 것. 배치 16이면 140대는 한 에폭에 9걸음(16x8=128, 남은 12대가)
# DataLoader = "섞어서 배치 크기만큼 하나씩 건네주는" 부품
from torch.utils.data import TensorDataset, DataLoader

# 습관 하나 추가: 정답(y)도 표준화합니다. 공정온도가 310근처라 0에서 출발하면 310까지 가는데 걸음이 많이 들고,
# 특히 Adam은 한 걸음 lr 크기(0.01)로 제한돼 310까지 3만걸음이 필요
# R2는 눈금을 바꿔도 같은 값이라 채점엔 영향이 없음
...
# 언제 뭘 쓰나?
#
# 표 데이터에 선형모델이나 트리 -> 사이킷런
# 데이터가 너무 커서 나눠 넣어야 하거나, 손실을 직접 만들어야 하거나, 층을 쌓는 신경망이면 -> 파이토치
# 파이토치를 배운 이유?
# 신경망 할 때 이 네줄이 그대로 나옴.
# 140대 한꺼번에 500걸음이나, 16대씩 900걸음(100에폭 x9)이나 같은 자리. 데이터가 커도 메모리엔 16대만 올리면 됨

# [메모리?]
# 메모리(RAM) = 작업 책상. 하드디스크 = 책장. 계산은 책상에 펼쳐 놓은 것만 할 수 있습니다
# 1) 파일이 컴퓨터에 저장돼 있는 건 '책장에 꽂힌' 상태.
#       read_csv로 읽어서 변수에 담으면 '책상에 펼친' 상태
# 2) 우리 데이터는 140행이라 책상에 다 펼쳐도 자리가 남습니다. 그래서 04까지는 한꺼번에 넣었음.
# 3) 100만행이면 책상이 모자랍니다. 다 펼치려는 순간 컴퓨터가 멈춥니다.
# 4) 그래서 16행만 펼쳤다 지우고, 다음 16행 펼쳤다가 지우고 -- 이게 미니배치입니다.
#       책장에는 100만 행이 있어도 책상 위에는 항상 16행만 있습니다.
#
# 숫자로 확인해 보면:
#   숫자 하나 = 8바이트. 100만행 x 컬럼 100개 = 약 0.8GB. 램이 16GB면 될 것 같습니다.
#   그런데 학습을 시작하면 기울기와 중간 계산값 사본이 몇 개씩 생겨서 3~5배가 됩니다. 그래서 터집니다.

# =====================================================================
# 7. 다음 과정 예고 — 층을 쌓으면 신경망 (지금은 한 줄만, 코드 없음)
# =====================================================================
# 지금까지 model4는 '직선 한 겹'(nn.Linear 하나)입니다. 다음 과정에서는 이 층을 여러 겹 쌓아 신경망을 만드는데
# 놀랍게도 위의 네 줄 루프(zero_grad -> loss -> backward -> step)는 한 글자도 안 바뀝니다. 층만 바뀜.
# 오늘 이 루프를 손에 익혀 두는 게 그 준비다.
...
for epochs in [5, 50, 500]:
    ...
    # test R² 계산
    with torch.no_grad():
        test_r2 = R2(Yte, model(Zte))

    print(f"{epochs}걸음 test R²: {test_r2:.4f}")
