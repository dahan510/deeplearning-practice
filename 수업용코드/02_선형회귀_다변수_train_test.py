...
# =====================================================================
# 01에선 입력이 공기온도 '하나'였습니다. 이번엔 네 개(공기온도 회전수 토크 공구마모)를 한꺼번에 봅니다.
# 바뀌는 건 딱 두가지에요
#
# 입력 하나 -> 입력 여러 개 : 예측 = w1x공기온도 + w2x회전수 + w3x토크 + w4x공가마모 +b
...
# =====================================================================
# 특징(feature) = 입력으로 쓰는 값 하나하나. 공기온도도 특징, 토큳 특징. X의 열 하나 = 특징 하나
# - 특징이 여러개인 선형회귀를 '다변수 선형회귀'라고 부릅니다. 이름만 길지 하는 일은 같음

# 왜 여러 개를 쓰나? - 온도 하나보다 온도+회전수+마모를 같이 보면 더 잘 맞을 수 있고
...
# =====================================================================
# 01에선 200대 전부로 배우고 200대 전부로 채점했습니다. 이건 '답안지 보고 푼 시험'이라 점수를 믿을 수 없음
# 그래서 시작하자마자 30% 떼어 금고에 넣고, 마지막 채점에만 꺼냅니다. 학습 중엔 절대 안 봄.
# train(학습용) = 배우는 데 쓰는 데이터(교과서)
...
print(f"    절편 b = {b:.3f}")
# 절편 310 = 모든 센서가 "평균값"일때의 예측 공정온도. 표준화했으니 z=0이 평균이라 그렇습니다.
...
# 과적합 = 학습데이터를 '외워서' 학습점수는 좋은데 새 데이터엔 틀리는 상태

# 과소적합 = 덜 배워서 학습 점수부터 나쁜 상태. 공부를 안 한 학생. 모의고사도 60점
#         - 과소적합 나면 -> 아예 못 맞힙니다. 센서를 더 넣거나, 더 오래 걷거나, 더 유연한 모델이 필요.
...
# =====================================================================
# 손잡이는 5개(w 4개 + b)인데 데이터가 5대면, 5대를 '완벽히' 지나는 답이 존재합니다. 외우기가 가능.
...
# 토크 마모 가중치가 140대 때보다 2~4배 커졌습니다.
# 5대 안에서 '우연히' 보인 패턴을 규칙으로 착각한 것.
# 04에서 배울 '규제(Ridge)'는 이 큰 가중치를 억눌러 과적합을 줄이는 도구이다.
...
for 뺄 in range(4):
    남길 = [
        j for j in range(4) if j != 뺄
    ]  # 리스트 컴프리핸션: 0~3 중 "뺄"만 뺀 번호 목록
...
# 공기온도를 빼면 학습용 0.05 / 시험용 0.00 - 둘 다 바닥. 이게 과소적합입니다.
# 중요한 센서가 빠져서 '배울 재료'가 없는 상태. 아무리 오래 걸려도 안 오릅니다. 해결은 센서 되돌려주기
#
# 다른 걸 빼면 거의 안 변합니다. -> 그 셋은 있어도 그만 없어도 그만. 가중치 순위와 같은 결론.
# 이렇게 '시험용 점수를 보면서' 센서를 고르는 건 원리르 보여주는 시연.
# 실무에선 시험용은 마지막에 딱 한 번만 보고, 고르는 일은 학습용 안에서 합니다.

# -- 오늘의 한 줄 --
# 입력이 여러 개면 X는 표, w는 여러 개, 예측은 Z@ w+b, 표준화는 얼마다, 학습용 통계로만.
# 채점은 시험용으로, 학습용과 나란히 보면 과적합(외움) 과소적합(못 배움)이 보임. 표준화 가중치가 중요도

# 다음시간 : 정답이 숫자가 아니라 '고장이냐 아니냐'면? -> 분류 로지스틱 회귀
...
print(f"    학습용 5대 R² {R2(y_train[:20], 예측(Z_train[:20], w5, b5)):.4f}")
print(f"    시험용 60대 R² {R2(y_test, 예측(Z_test, w5, b5)):.4f}")
print("    가중치도 달라짐:", w5.round(2), "← 140대 때는", w.round(2))
